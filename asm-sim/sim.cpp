  ...
  while(!quit)
  {
    ...
    switch((char)insn.itype.opcode) // the cast is just to suppress warnings about the unsupported opcodes
    {
      ...
      case OPCODE_JAL:
        ...
        break;
      case OPCODE_OUTPUT:
        if (reg[insn.rtype.rs] >= 'A' && reg[insn.rtype.rs] <= 'Z'
        || reg[insn.rtype.rs] >= 'a' && reg[insn.rtype.rs] <= 'z'
        || reg[insn.rtype.rs] >= '0' && reg[insn.rtype.rs] <= '9'
        || reg[insn.rtype.rs] == '\n')
          cout << (char)reg[insn.rtype.rs];
        else
